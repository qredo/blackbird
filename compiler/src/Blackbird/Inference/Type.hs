...
--
--------------------------------------------------------------------
module Blackbird.Inference.Type
  ( matchFunType
  , inferBindings
  , inferType
  , inferPatternType
...
import Blackbird.Builtin.Type as Type
import Blackbird.Constraint.Env
import Blackbird.Constraint.Simplification
import Blackbird.Diagnostic (diePretty)
import Blackbird.Pattern.Env as Pattern
import Blackbird.Pattern.Matching as Pattern
import Blackbird.Pretty (names)
import Blackbird.Pretty.Core (prettyCore)
import Blackbird.Pretty.Meta
import Blackbird.Pretty.Term (prettyTerm)
...
import Blackbird.Syntax.Pattern as Pattern
import Blackbird.Syntax.Scope
import Blackbird.Syntax.Term as Term
import qualified Blackbird.Syntax.Type as Type
import Blackbird.Syntax.Type hiding (Var, Loc)
import Blackbird.Inference.Kind
import Blackbird.Inference.Witness
import Blackbird.Unification.Type
...
matchFunType :: MonadMeta s m => TypeM s -> m (Maybe (TypeM s, TypeM s))
matchFunType (Type.App (Type.App (HardType Arrow) a) b) = return $ Just (a, b)
matchFunType (HardType _) = pure Nothing
matchFunType t = do
  bx <- pure <$> newMeta True Nothing
  by <- pure <$> newMeta True Nothing
  x <- pure <$> newMeta (Type bx) Nothing
  y <- pure <$> newMeta (Type by) Nothing
  Just (x, y) <$ unsharingT (unifyType t (x ~> y))
...
inferBindingType
  :: (MonadConstraint (KindM s) s m, Show v)
...
inferBindingType d cxt lcxt bdg = do
  ...
  let rs       = mess^.folded._1
      guards   = view _3 <$> mess
      wheres   = view _4 <$> mess
      (t0:tys) = view _2 <$> mess -- TODO be safe
...
zonkKinds :: MonadMeta s m => Typ (MetaK s) t -> m (Typ (MetaK s) t)
zonkKinds = fmap (bindType id pure) . kindVars (zonk_.pure)

defaultQuantifiers :: MonadMeta s m => Fold k (MetaK s) -> Typ k t -> m ()
...
inferType :: (MonadConstraint (KindM s) s m, Show v)
...
inferType d cxt (Term.App f x) = do
  ...
  (i, o) <- matchFunType ft >>= flip maybe pure (do
    r <- viewMeta metaRendering
    xd <- prettyTerm x names (-1) (\_ _ -> pure "...") (\_ _ -> pure "...")
    throwM . diePretty r $ "extra words:" <+> xd)
...
inferTypeInScope :: (MonadConstraint (KindM s) s m, Show b, Show v)
...
inferAltTypes :: (MonadConstraint (KindM s) s m, Show v)
...
coalesceGuarded (Guarded l) = do
  ...
  (rs, ty, l') <- foldrM ?? ([], rt, []) ?? l $
    \(Pair (Witness gr gt gc) (Witness br bt bc)) (rs, ty, gcs) -> do
      uncaring $ unifyType gt Type.bool
      (rs++gr++br,,Pair gc bc:gcs) <$> runSharing ty (unifyType ty bt)
...
abstractedWitness :: (MonadConstraint (KindM s) s m, Show a)
...
abstractedWitness d sks rs cs0 ty co0 = do
  ...
  unless (Foldable.all (`Foldable.elem` cs) co') $ do
    co'' <- bitraverse (pure . const C) pure $ fromScope co'
    pco <- prettyCore names (-1) (unvar (\a d' -> pure . pretty . showsPrec d' a $ "") (prettyTypeM ?? names)) co''
    pcs <- traverse (\ty' -> prettyTypeM ty' names (-1)) cs
    fail $ "[E000] undischarged obligation\n" ++ show pco ++ "\n" ++ show pcs
  ((rs', ty'), co'') <-
...
checkType :: (MonadConstraint (KindM s) s m, Show v)
...
subsumesType :: (MonadConstraint (KindM s) s m, Show v)
...
subsumeAndGeneralize d wts = do
  ...
  for mess $ \(sks, sts, rs, cs0, ty, signed, co0) -> do
    ...
    co' <- simplifyVia cs co
    unless (not signed || Foldable.all (`Foldable.elem` cs) co') $ fail "[E001] undischarged obligation"
...
generalizeType :: MonadMeta s m => WitnessM s a -> m (Typ k t, Core Convention a)
generalizeType w = do
...
inferHardType (Term.Lit l@Person{}) =
  return $ Witness [] (literalType l) $
             dataCon [_Convention # N] 0 Type.policy ## (_Lit # l)
...
inferHardType Hole = do
  ...
  r <- viewMeta metaRendering
  return $ Witness [] (Type.Var tv) $ _HardCore # (Core.Error $ SText.pack $ show $ unAnnotate $ explain r $ Err (Just "open hole") [] mempty [])
inferHardType (DataCon g t)
  ...
  | g^.name == "[]"      = unfurl (bimap absurd absurd t) $ dataCon [] 0 g
  | g^.name == "::"      = unfurl (bimap absurd absurd t) $ dataCon [_Convention # C, _Convention # C] 1 g
inferHardType _ = fail "Unimplemented"

literalType :: Literal -> Typ k a
...
literalType Person{} = Type.policy
...
inferPatternType d (ConP g ps)
  ...
  | g^.name == "[]" =
    case ps of
      [] -> newShallowMeta d star (Just "a") >>= \x ->
              return ([], Type.list $ pure x, error "panic: bad pattern path")
      _ -> fail "over-applied constructor"
  | g^.name == "::" =
    case ps of
      [     ] -> fail "under-applied constructor"
      [_    ] -> fail "under-applied constructor"
      [p,ps'] -> newShallowMeta d star (Just "a") >>= \x -> do
        (sks, ty, f) <- inferPatternType d p
        uncaring $ unifyType (pure x) ty
        (sks', ty', f') <- inferPatternType d ps'
        uncaring $ unifyType (Type.list $ pure x) ty'
        return (sks ++ sks'
               , Type.list $ pure x
               , \xs -> case xs of
                   FieldPP 0 pp -> f pp
