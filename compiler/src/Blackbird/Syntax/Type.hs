...
import Blackbird.Syntax.Kind hiding (Var, general)
import qualified Blackbird.Syntax.Kind as Kind
...
data Typ k a
  ...
  | Loc !Rendering !(Typ k a)
  | Exists [Hint] [(Hint, Scope Int Kind k)] (Scope Int (TK k) a)
  | And [Typ k a]
  deriving (Show, Read, Functor, Foldable, Traversable, Generic, Generic1)
  deriving (Show1, Read1) via (Generically1 (Typ k))
...
instance (p ~ Tagged, f ~ Identity) => Tup p f (Typ k t) where
  _Tup = unto hither
   where hither [x] = x
         hither l = apps (HardType . Tuple $ length l) l

instance (Data k, Data a) => Data (Typ k a) where
  ...
  dataTypeOf _ = mkNoRepType "Blackbird.Syntax.Type"
  dataCast1 f = gcast1 f
  dataCast2 f = gcast2 f

instance Digestable k => Digestable1 (Typ k)
...
-- get these from Hashable2 both for general consistency and because we have to
-- hash Scope by hand in Hashable2
instance (Hashable k, Hashable a) => Hashable (Typ k a) where
  hashWithSalt = liftHashWithSalt2 hashWithSalt hashWithSalt
instance (Hashable k) => Hashable1 (Typ k) where
  liftHashWithSalt = liftHashWithSalt2 hashWithSalt

instance Hashable2 Typ where
  liftHashWithSalt2 f g n = \case
    Var a             -> g n a
    App l r           -> h n l `h` r `hashWithSalt` distApp
    HardType t        -> hashWithSalt n t `hashWithSalt` distHardType
    Forall k tvs cs b -> hashWithSalt n k `htv` tvs `hs` cs `hs` b `hashWithSalt` distForall
    Loc _ ty          -> h n ty
    Exists k tvs cs   -> hashWithSalt n k `htv` tvs `hs` cs `hashWithSalt` distExists
    And xs            -> liftHashWithSalt h n xs `hashWithSalt` distAnd
   where
    infixl 0 `h`
    h = liftHashWithSalt2 f g
    infixl 0 `hs`
    hs n' s = liftHashWithSalt2 (liftHashWithSalt f) (liftHashWithSalt g) n' (fromScope s)
    infixl 0 `htv`
    -- Int -> [(Hint, Scope Int Kind a)] -> Int
    htv = liftHashWithSalt (liftHashWithSalt (liftHashWithSalt f))


instance (Digestable k, Digestable t) => Digestable (Typ k t) where
...
instance IsString a => IsString (Typ k a) where
  fromString = Var . fromString
  {-# INLINE fromString #-}

instance Variable (Typ k) where
  _Var = prism Var $ \t -> case t of
    Var a -> Right a
    _     -> Left  t
  {-# INLINE _Var #-}

instance Typical (Typ k a) where
  hardType = prism HardType $ \ s -> case s of
    HardType a -> Right a
    _          -> Left s
  {-# INLINE hardType #-}

instance Eq2 Typ where
  liftEq2 f g (Loc _ l         ) (r                   ) = liftEq2 f g l r
  liftEq2 f g (l               ) (Loc _ r             ) = liftEq2 f g l r
  liftEq2 _ g (Var a           ) (Var b               ) = g a b
  liftEq2 f g (App l r         ) (App l' r'           ) = liftEq2 f g l l' && liftEq2 f g r r'
  liftEq2 _ _ (HardType x      ) (HardType y          ) = x == y
  liftEq2 f g (Forall n ks cs b) (Forall n' ks' cs' b') = n == n' && liftEq (liftEq (liftEq f)) ks ks' && liftEq2H (liftEq2 (liftEq f)) g cs cs' && liftEq2H (liftEq2 (liftEq f)) g b b'
  liftEq2 f g (Exists n ks cs  ) (Exists n' ks' cs'   ) = n == n' && liftEq (liftEq (liftEq f)) ks ks' && liftEq2H (liftEq2 (liftEq f)) g cs cs'
  liftEq2 f g (And cs          ) (And cs'             ) = liftEq (liftEq2 f g) cs cs'
  liftEq2 _ _ _                   _                     = False

instance Bifunctor Typ where
  bimap = bimapDefault
  {-# INLINE bimap #-}

instance Bifoldable Typ where
  bifoldMap = bifoldMapDefault
  {-# INLINE bifoldMap #-}

instance Bitraversable Typ where
...
instance HasKindVars (Typ k a) (Typ k' a) k k' where
  kindVars f = bitraverse f pure

instance Eq k => Eq1 (Typ k) where liftEq = liftEq2 (==)

instance (Eq k, Eq a) => Eq (Typ k a) where (==) = liftEq2 (==) (==)

instance Applicative (Typ k) where
  pure = Var
  {-# INLINE pure #-}
  (<*>) = ap
  {-# INLINE (<*>) #-}

instance Monad (Typ k) where
  m >>= g = bindType Kind.Var g m
  {-# INLINE (>>=) #-}

instance Serial2 Typ where
...
forAll :: (Ord k, Ord t)
       => (k -> Hint) -> (t -> Hint) -> [k] -> [(t, Kind k)] -> Typ k t -> Typ k t
       -> Typ k t
...
-- are maintained.
mergeConstraints :: Typ k t -> Typ k t -> Typ k t
...
allConstraints :: [Typ k t] -> Typ k t
...
isTrivialConstraint :: Typ k a -> Bool
...
isRowConstraint :: Typ k a -> Bool
isRowConstraint _ = False

-- | Perform simultaneous substitution on kinds and types in a 'Typ'.
bindType :: (k -> Kind k') -> (a -> Typ k' b) -> Typ k a -> Typ k' b
...
closedType :: Typ k t -> Maybe (Typ k' t')
...
instance HasTypeVars (Typ k a) (Typ k b) a b where
  typeVars = traverse
  {-# INLINE typeVars #-}
...
-- | Witnesses the isomorphism between 'Typ Int t' and 'Scope t (TK Void) Void'
trivialTK
  :: Iso (Scope b (TK Void) Void) (Scope b' (TK Void) Void) (Typ Int b) (Typ Int b')
trivialTK = iso (bimap trivVar trivVar . fromScope) (toScope . bimap B B)
 where trivVar = unvar id absurd

-- | Bind some of the kinds referenced by a 'Typ'.
...
-- that doesn't matter.
abstractKinds :: (k -> Maybe Int) -> Typ k a -> TK k a
...
abstractAll :: (Ord k, Ord a)
            => (k -> Hint) -> (a -> Hint)
            -> Typ (Maybe k) a -> (Scope Int (TK ()) b, ([Hint], [Hint]))
...
-- | Instantiate the kinds bound by a 'TK' obtaining a 'Typ'.
instantiateKinds :: (Int -> Kind k) -> TK k a -> Typ k a
...
instantiateKindVars :: [k] -> TK k a -> Typ k a
...
instance BoundBy (Annot k) (Typ k) where
