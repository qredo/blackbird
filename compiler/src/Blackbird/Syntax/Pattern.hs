...
--
--------------------------------------------------------------------
module Blackbird.Syntax.Pattern
  ( Pattern(..)
  , _SigP
  , _WildcardP
  , _AsP
...
-- TODO be safe
_LitP' :: HasCallStack => Literal -> Prism' (Pattern t) [Pattern t]
_LitP' l = prism (\case {[] -> LitP l ; _ -> error "_LitP' with arguments"}) $ \xs -> case xs of
  LitP l' | l == l' -> Right []
  p -> Left p
...
data Pair a = Pair a a
  ...
  deriving (Eq1,Show1) via (Generically1 Pair)
...
data Guarded tm
  ...
  | Guarded [Pair tm]
  deriving (Eq, Ord, Show, Read, Functor, Foldable, Traversable, Generic, Generic1)
  deriving (Eq1,Show1) via (Generically1 Guarded)
...
data Alt t f a = Alt !(Pattern t) (Guarded (Scope PatternPath f a))
  ...
  deriving (Eq1,Show1) via (Generically1 (Alt t f))
...
instance Serial1 Guarded where
  ...
  serializeWith ptm (Guarded tms)  = putWord8 1 *> serializeWith putPair tms where
    putPair (Pair tm1 tm2) = ptm tm1 *> ptm tm2

  deserializeWith gtm = getWord8 >>= \b -> case b of
    0 -> Unguarded <$> gtm
    1 -> Guarded <$> deserializeWith (Pair <$> gtm <*> gtm)
