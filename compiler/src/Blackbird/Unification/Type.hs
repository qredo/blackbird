...
--
--------------------------------------------------------------------
module Blackbird.Unification.Type
  ( unifyType
  , zonkKindsAndTypes
...
import Blackbird.Inference.Kind (checkKind)
...
typeOccurs depth1 t p = zonkKindsAndTypesWith t tweakDepth tweakType where
  ...
  tweakType m
    | p m = do
      ...
      throwM $ die r "infinite type detected" & footnotes .~ ["cyclic type:" <+> hang 4 (group (pretty v </> pretty '=' </> td))]
