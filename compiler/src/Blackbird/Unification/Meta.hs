...
-- Meta variables
--------------------------------------------------------------------
module Blackbird.Unification.Meta
  (
  -- * Meta variables
    Meta(Meta)
...
type MetaT s = Meta s (Typ (MetaK s)) (KindM s)

-- | A type filled with meta-variables
type TypeM s = Typ (MetaK s) (MetaT s)
...
-- | Path-compression
semiprune :: (Variable f, MonadMeta s m) => f (Meta s f a) -> m (f (Meta s f a))
...
class (MonadFail m, MonadST m, s ~ World m) => MonadMeta s m | m -> s where
  askMeta :: m (MetaEnv s)
#ifndef HLINT
...
instance MonadMeta s m => MonadMeta s (ReaderT t m) where
  localMeta f (ReaderT m) = ReaderT (localMeta f . m)
