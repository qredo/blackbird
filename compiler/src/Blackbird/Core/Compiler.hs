...
module Blackbird.Core.Compiler
  ( SortRef(..)
  , compile
  , compileBinding
  , compileBranches
...
import Data.Word
import Blackbird.Native.Policy (_Policy_Signature, _Address_Cooked)
import Blackbird.Syntax.G
import Blackbird.Syntax.Convention as C
import Blackbird.Syntax.Core (Core, HardCore, Match(..))
import Blackbird.Syntax.Literal
import qualified Blackbird.Syntax.Core as Core
import Blackbird.Syntax.Sort as S
...
localSorts :: [(v,SortRef)] -> ([(v, SortRef)], Sorted Word64)
...
compileBranches n ev cxt bs d = Continuation bs' d'
 ...
 bs' = bs <&> \(Match ccvs _ e) ->
   let cxt' :: Var Word64 v -> SortRef
       cxt' = unvar bc $ cxt & mapped.sortRef._Stack %@~ \s r -> r + fields^.sort s
...
anf cxt s = cleanup $ runState (traverse (uncurry compilePiece) s) (0, []) where
  ...
  compilePiece C.N (Core.HardCore (Core.Lit (Person i))) =
    return (False, SortRef S.N $ _UnsafeNative . _Policy_Signature . _Address_Cooked # (mempty, i))
