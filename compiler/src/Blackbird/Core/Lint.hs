...
liftMaybe :: MonadFail m => String -> Maybe b -> m b
...
inferHardCore (Lit Person {}) = return $ Form [] N
...
bindings cs = variables %~ unvar (\i -> maybeToRight "illegal bound variable" $ cs^?ix (fromIntegral i))
