...
import Data.Set (Set)
import Data.Void
import Prettyprinter as Pretty hiding (line, (<>), braces)
...
data Failure = Failure
  { _message  :: [Doc AnsiStyle]
  , _expected :: Set String
  } deriving Show

instance Semigroup Failure where
  Failure as xs <> Failure bs ys = Failure (as <|> bs) (xs <> ys)
...
instance MonadFail Parser where
  fail e = Parser $ \_ -> Fail (Failure [pretty e] mempty)
...
race (Parser m) (Parser n) = Parser $ \s -> case m s of
  ...
  l@(Commit a t xs) -> case n s of -- old state
    r@(Commit _a' t' xs') -> case _parserLocation t `compare` _parserLocation t' of
      LT -> r
      EQ -> Commit a t (xs <> xs')
      GT -> l
    Err el e
...
instance Parsing Parser where
  ...
  notFollowedBy (Parser m) = Parser $ \s -> case m s of
    Pure a _     -> Fail  (Failure [pretty "unexpected" <+> pretty (show a)] mempty)
    Commit a t _ -> err t (Failure [pretty "unexpected" <+> pretty (show a)] mempty)
    _            -> Pure () mempty
  unexpected s = Parser $ \_ -> Fail (Failure [pretty "unexpected" <+> pretty s] mempty)
