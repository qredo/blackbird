...
module Blackbird.Loader.Filesystem
  ( filesystemLoader
  , LoadRefusal(..)
  , explainLoadRefusal
  , Freshness()
...
filesystemLoader :: forall m. (MonadIO m, MonadError LoadRefusal m) =>
...
filesystemLoader root ext =
  testBasedCacheLoader pathName
      ...
      ((/=) `on` modTime)
  where pathName :: Text -> m P.FilePath
        pathName = hoistEither . runExcept
                   . fmap (\n -> root </> n <.> ext)
                   . moduleFileName
...
invalidChars t = [(-1, "Can't be empty") | T.null t ]
  ...
  where checkChars :: Text -> [(Int, Text)]
        checkChars = flip evalState False . runApMonoid
