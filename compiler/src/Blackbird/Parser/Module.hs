...
module Blackbird.Parser.Module
...
import Blackbird.Builtin.Term hiding (explicit)
import Blackbird.Parser.Data (dataType)
import Blackbird.Parser.Resolver
import Blackbird.Parser.Style
import Blackbird.Parser.Term
import Blackbird.Parser.Trifecta (LayoutParsing(layouted), braces)
import Blackbird.Parser.Type (Ann, annotation)
import Blackbird.Syntax.Global (Fixity(..), Assoc(..))
import Blackbird.Syntax.Module hiding (explicit, fixityDecl, moduleHead)
import Blackbird.Syntax.ModuleName
import qualified Blackbird.Syntax.Term as Term
import Text.Parser.Char
import Text.Parser.Combinators
import Text.Parser.Token hiding (braces)

-- | Parse the whole file, but only the module head.
moduleHead :: (Monad m, LayoutParsing m) => m (ModuleHead (Import Text) Text)
...
wholeModule :: (MonadFail m, MonadPlus m, LayoutParsing m) =>
...
useNames names =
  ...
  where nameIndex = HM.fromList (explicitName <$> names)
        ...
        explicitName ex = let gn :: Text
                              gn = ex^.explicitGlobal
...
imports :: (Monad m, LayoutParsing m) => m [Import Text]
imports = bracedSemiSep importExportStatement <?> "import statements"

importExportStatement :: forall m. (Monad m, LayoutParsing m) => m (Import Text)
importExportStatement =
  ...
  where
  impList :: m [Explicit Text]
  impList = bracedSemiSep explicit <?> "explicit imports"
...
explicit :: forall m. (Monad m, TokenParsing m) => m (Explicit Text)
explicit = do
  isTy <- option False (True <$ symbol "type")
  let nam :: m Text
      nam = operator <|> (if isTy then ident typeCon
...
assembleBindings :: (MonadFail m, Ord a, Show a) =>
                    [(Privacy, [a], Ann)]             -- ^ types
                 -> [(Privacy, a, NonEmpty (PreBody Ann a))]   -- ^ terms
                 -> m [(Privacy, a, Term.Binding Ann a)] -- ^ bindings
assembleBindings typs trms = do
  typs' <- foldM (\m (p, ts, a) -> foldM (\acc t -> insertGuarded t (p, a) acc) m ts) M.empty typs
  trms' <- foldM (\m (privy, t, bdy) -> insertGuarded t (privy, bdy) m) M.empty trms
  assembleBindings' typs' trms' where
    insertGuarded k a m | M.member k m = fail   $ "multiple bindings with names(s): " ++ show k
                        | otherwise    = return $ M.insert k a m

assembleBindings' :: (Show k, Ord k, MonadFail m, Eq a) =>
                     M.Map k (a, t)
                  -> M.Map k (a, NonEmpty (PreBody t k))
                  -> m [(a, k, Term.Binding t k)]
assembleBindings' typs trms = foldM f [] termList where
  f acc (nam, (p, ts)) =
  ...
  termList = M.toList trms

statements :: (MonadState s m, HasFixities s, MonadFail m, LayoutParsing m) =>
...
statement :: (MonadState s m, HasFixities s, MonadFail m, LayoutParsing m) =>
             m (Statement Text Text)
...
fixityDecl :: forall m. (Monad m, TokenParsing m) => m FixityDecl
fixityDecl = FixityDecl
         <$> option TypeLevel (TermLevel <$ symbol "type")
         <*> (fixity <*> prec)
         <*> many operator
  where fixity :: m (Int -> Fixity)
        fixity = Infix L <$ symbol "infixl"
...
sigs :: (MonadFail m, TokenParsing m) => m ([Text], Ann)
sigs = (,) <$> try (termIdentifier `sepBy` comma <* colon)
           <*> annotation

termStatement :: (MonadState s m, HasFixities s, MonadFail m, LayoutParsing m) => m (Text, (NonEmpty (PreBody Ann Text)))
termStatement = do
...
-- | Parse { ma; ... ma }
bracedSemiSep :: LayoutParsing m => m a -> m [a]
bracedSemiSep p = colon *> layouted (semiSep p) <|> braces (semiSep p)
...
fetchGraphM :: forall a k m. (Hashable k, Monad m)
            => (a -> [k])         -- ^ Dependencies.
